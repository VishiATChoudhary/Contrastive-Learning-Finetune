...
#######CODE SNIPPET TO SAVE PREDICTIONS#######
        #check if preds.csv exists, if not create it
        try:
            preds_df = pd.read_csv('preds.csv')
        except:
            preds_df = pd.DataFrame(columns=['preds', 'y'])

        #append preds and y to preds.csv
        try: 
            preds_df = pd.concat([preds_df, pd.DataFrame({'preds':preds, 'y':y})], axis=0)
        except:
            preds_df = pd.concat([preds_df, pd.DataFrame({'preds': preds.tolist(), 'y': y.tolist()})], axis=0, ignore_index=True)
...
####################################################
    ...
    def num_training_steps(trainer, dm) -> int:
        ...
        num_devices = 1 #max(1, trainer.num_gpus, trainer.num_processes)
